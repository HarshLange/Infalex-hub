...
export function PricingPreview({ pricing }: { pricing?: ProductPricing[] }) {
  ...
  return (
    <section className="py-24 bg-bg border-b border-border relative overflow-hidden">
      <div className="absolute top-0 right-0 w-full max-w-3xl h-full bg-primary/5 blur-[150px] pointer-events-none -z-10" />
          ...
          {pricing.map((plan, idx) => (
            <div 
              ...
              className={cn(
                ...
                plan.highlighted 
                  ? "bg-surface border-primary shadow-xl shadow-primary/10 relative" 
            ...
            >
              ...
              <h3 className="text-xl font-bold text-foreground mb-2">{plan.planName}</h3>
              <p className="text-sm text-foreground-muted mb-6 min-h-[40px]">{plan.description}</p>
              
              <div className="mb-8">
                <span className="text-4xl font-bold text-foreground tracking-tight">{plan.price}</span>
                {plan.price !== "$0" && <span className="text-foreground-muted">/month</span>}
              </div>
              ...
              <ul className="flex flex-col gap-4 mt-auto">
                {plan.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-3 text-sm text-foreground-muted">
